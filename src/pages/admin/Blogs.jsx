...
import { HiPlus, HiX, HiCalendar, HiLink, HiVideoCamera } from "react-icons/hi";
...
export default function Blogs() {
    ...
    return (
        <div className="relative">
            ...
            <div className="grid lg:grid-cols-3 gap-10 h-[calc(100vh-200px)] overflow-hidden">

                {/* LEFT COLUMN: FORM (MODAL ON MOBILE) */}
                <div className={`
                    ${isModalOpen ? "fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md" : "hidden lg:block lg:col-span-1"}
                `}>
                    <div className="bg-white lg:bg-transparent w-full max-w-xl lg:max-w-none p-6 lg:p-0 rounded-[2.5rem] relative max-h-[90vh] lg:max-h-full flex flex-col shadow-2xl lg:shadow-none">
                        ...
                        <button onClick={() => setIsModalOpen(false)} className="lg:hidden absolute top-4 right-6 text-slate-400 hover:text-rose-500 transition-colors">
                            <HiX size={24} />
                        </button>
                        ...
                        <form className="space-y-4 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner overflow-y-auto scrollbar-hide" onSubmit={handleSubmit}>
                            ...
                            <div className="relative">
                                <input
                                    type="date"
                                    className="w-full p-4 pl-12 rounded-2xl border-none shadow-sm text-slate-500 focus:ring-2 focus:ring-indigo-500"
                                    value={form.date}
                            ...
                            <button
                                disabled={loading}
                                className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 transition-all disabled:opacity-50"
                            >
                ...
                {/* RIGHT COLUMN: LIST */}
                <div className="lg:col-span-2 h-full overflow-y-auto scrollbar-hide pb-20 pr-2">
                            ...
                            <div key={b.id} className="bg-white p-5 rounded-[2.5rem] border border-slate-100 flex flex-col sm:flex-row gap-6 hover:shadow-xl transition-all group overflow-hidden">
                                ...
                                <div className="flex-1 py-2 flex flex-col">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <span className="shrink-0 text-[10px] font-black bg-indigo-50 text-indigo-500 px-3 py-1 rounded-full uppercase tracking-tighter">
                                            {b.date}
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex gap-4 mt-auto pt-4 border-t border-slate-50">
                                        ...
                                        <button
                                            onClick={async () => { if (confirm("Delete this post?")) { await supabase.from("blogs").delete().eq("id", b.id); fetchBlogs(); } }}
                                            className="text-rose-500 font-black text-xs uppercase hover:underline"
                                        >
                                            Delete
                                        </button>
                                    </div>
