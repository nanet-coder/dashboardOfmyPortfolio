            ...
            <div className="grid lg:grid-cols-3 gap-10 h-[calc(100vh-220px)] overflow-hidden">
                ...
                <div className={`${isModalOpen ? "fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" : "hidden lg:block lg:col-span-1"}`}>
                    <div className="bg-white lg:bg-transparent w-full max-w-xl p-6 lg:p-0 rounded-[2.5rem] relative max-h-[90vh] overflow-y-auto scrollbar-hide">
                        {/* Close button for desktop/mobile */}
                        <button onClick={() => { setIsModalOpen(false); resetForm(); }} className="absolute top-3 right-3 text-slate-400 hover:text-rose-500 transition-colors">
                            <HiX size={24} />
                        </button>

                        <form className="space-y-4 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner" onSubmit={handleSubmit}>
                            <h2 className="text-xl font-black text-slate-800">{editingId ? "✨ Edit Post" : "✍️ New Blog"}</h2>
                            <input type="text" placeholder="Post Title" className="w-full p-4 rounded-2xl border-none shadow-sm" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} required />

                            <div className="relative">
                                <input type="date" className="w-full p-4 pl-12 rounded-2xl border-none shadow-sm text-slate-500" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} required />
                            ...
                            {videoMode === "url" ? (
                                <div className="relative">
                                    <input type="url" placeholder="Video Link" className="w-full p-4 pl-12 rounded-2xl border-none shadow-sm" value={form.video_url} onChange={e => setForm({ ...form, video_url: e.target.value })} />
                            ...
                            <textarea placeholder="Write your content..." className="w-full p-4 rounded-2xl border-none h-32 shadow-sm" value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} required />

                            <button disabled={loading} className="w-full py-4 bg-indigo-600 text-white font-black rounded-2xl shadow-xl hover:bg-indigo-700 transition-all">{loading ? "PROCESSING..." : editingId ? "UPDATE POST" : "PUBLISH POST"}</button>
                        </form>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2 flex flex-col h-full overflow-hidden">
                    <div className="relative mb-6">
                        <input
                            ...
                            className="w-full p-4 pl-12 bg-white rounded-3xl border-none shadow-sm"
                            value={searchTerm}
